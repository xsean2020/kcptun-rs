...
- `kcp.rs` — KCP state machine (Go kcp-go v5 port): `send`/`input_no_flush`/`input_no_flush_typed`/`flush_with_current`; `acklist: SmallVec` ; `pending_flush` batches one flush per datagram burst; cached monotonic clock (`current_ms`). `input_no_flush_typed(data, regular, ack_no_delay)` carries Go's `pktType`: FEC-reconstructed datagrams pass `regular = false` and then skip the `rmt_wnd` update, the RTT sample and `RepeatSegs`.
...
This project is indexed by GitNexus as **kcptun-rs** (6202 symbols, 13922 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
